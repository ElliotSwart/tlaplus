...
//    this.isATuple = false;
     ...
     */
    public final void setArgs(final ExprOrOpArgNode[] args) {
        this.operands = args;
    }
...
//      this.argLevelConstraints = new SetOfArgLevelConstraints();
    ...
    public void walkGraph(final Hashtable<Integer, ExploreNode> semNodesTable, final ExplorerVisitor visitor) {
        ...
        if (operands != null && operands.length > 0) {
            for (final ExprOrOpArgNode operand : operands)
                if (operand != null) operand.walkGraph(semNodesTable, visitor);
        }
    ...
    public String toString(final IValue aValue) {
        if (aValue instanceof ITupleValue && allParams.size() == aValue.size()) {
            ...
            final TreeSet<SymbolNode> s = new TreeSet<>(Comparator.comparingInt(o -> o.getName().getLoc()));
            s.addAll(allParams);
