package tlc2.output;

import java.util.Set;
import java.util.stream.Collectors;
import java.util.stream.Stream;

/**
 * Interface containing the error code constants 
 * @author Simon Zambrovski
 * @version $Id$
 */
public interface EC
{
    // This is reserved so that an optional error code can be safely represented in a single int
    int NO_ERROR = 0;
    
    // Check and CheckImpl
    // check if the TLC option is the same for params
    int CHECK_FAILED_TO_CHECK = 3000;
    int CHECK_COULD_NOT_READ_TRACE = 3001;

    int CHECK_PARAM_EXPECT_CONFIG_FILENAME = 3100;
    int CHECK_PARAM_USAGE = 3101;
    int CHECK_PARAM_MISSING_TLA_MODULE = 3102;
    int CHECK_PARAM_NEED_TO_SPECIFY_CONFIG_DIR = 3103;
    int CHECK_PARAM_WORKER_NUMBER_REQUIRED = 3104;
    int CHECK_PARAM_WORKER_NUMBER_TOO_SMALL = 3105;
    int CHECK_PARAM_WORKER_NUMBER_REQUIRED2 = 3106;
    int CHECK_PARAM_DEPTH_REQUIRED = 3107;
    int CHECK_PARAM_DEPTH_REQUIRED2 = 3108;
    int CHECK_PARAM_TRACE_REQUIRED = 3109;
    int CHECK_PARAM_COVREAGE_REQUIRED = 3110;
    int CHECK_PARAM_COVREAGE_REQUIRED2 = 3111;
    int CHECK_PARAM_COVREAGE_TOO_SMALL = 3112;
    int CHECK_PARAM_UNRECOGNIZED = 3113;
    int CHECK_PARAM_TOO_MANY_INPUT_FILES = 3114;

    
    int SANY_PARSER_CHECK_1 = 4000;
    int SANY_PARSER_CHECK_2 = 4001;
    int SANY_PARSER_CHECK_3 = 4002;

    int UNKNOWN = -1;  // TODO remove all these
    int UNIT_TEST = -123456;
	/**
	 * A feature of TLA+/TLC is not supported by TLC's current mode. E.g.
	 * TLCGet/TLCSet operator cannot be used in distributed TLC.
	 */
    int TLC_FEATURE_UNSUPPORTED = 2156;
	int TLC_FEATURE_UNSUPPORTED_LIVENESS_SYMMETRY = 2279;
	int TLC_FEATURE_LIVENESS_CONSTRAINTS = 2284;

    int GENERAL = 1000;
    int SYSTEM_OUT_OF_MEMORY = 1001;
    int SYSTEM_OUT_OF_MEMORY_TOO_MANY_INIT = 1002;
    int SYSTEM_OUT_OF_MEMORY_LIVENESS = 1003;
    int SYSTEM_STACK_OVERFLOW = 1005;

    int WRONG_COMMANDLINE_PARAMS_SIMULATOR = 1101;
    int WRONG_COMMANDLINE_PARAMS_TLC = 1102;

    int TLC_PP_PARSING_VALUE = 2000;
    int TLC_PP_FORMATING_VALUE = 2001;

    int TLC_METADIR_EXISTS = 2100;
    int TLC_METADIR_CAN_NOT_BE_CREATED = 2101;
    int TLC_INITIAL_STATE = 2102;
    int TLC_NESTED_EXPRESSION = 2103;
    int TLC_ASSUMPTION_FALSE = 2104;
    int TLC_ASSUMPTION_EVALUATION_ERROR = 2105;
    int TLC_STATE_NOT_COMPLETELY_SPECIFIED_INITIAL = 2106;

    int TLC_INVARIANT_VIOLATED_INITIAL = 2107;
    int TLC_PROPERTY_VIOLATED_INITIAL = 2108;
    int TLC_STATE_NOT_COMPLETELY_SPECIFIED_NEXT = 2109;
    int TLC_INVARIANT_VIOLATED_BEHAVIOR = 2110;
    int TLC_INVARIANT_EVALUATION_FAILED = 2111;
    int TLC_INVARIANT_VIOLATED_LEVEL = 2146;
    int TLC_ACTION_PROPERTY_VIOLATED_BEHAVIOR = 2112;
    int TLC_ACTION_PROPERTY_EVALUATION_FAILED = 2113;
    int TLC_DEADLOCK_REACHED = 2114;

    int TLC_STATES_AND_NO_NEXT_ACTION = 2115;
    int TLC_TEMPORAL_PROPERTY_VIOLATED = 2116;
    int TLC_FAILED_TO_RECOVER_NEXT = 2117;
    int TLC_NO_STATES_SATISFYING_INIT = 2118;
    int TLC_STRING_MODULE_NOT_FOUND = 2119;

    int TLC_ERROR_STATE = 2120;
    int TLC_BEHAVIOR_UP_TO_THIS_POINT = 2121;
    
    int TLC_STATE_NOT_COMPLETELY_SPECIFIED_LIVE = 2148;
    
    /**
     * This error code accompanies the final state in a liveness property
     * counter-example trace, when the trace takes the form of a lasso
     * (so the accompanied state is a state we are going "back to").
     * For traces ending in stuttering, see {@link EC#TLC_STATE_PRINT3}.
     * Note this code only accompanies the *final* state in the trace,
     * and the prefix traces are accompanied by the
     * {@link EC#TLC_STATE_PRINT2} code.
     */
    int TLC_BACK_TO_STATE = 2122;

    /**
     * This error code accompanies the final state in a liveness property
     * counter-example trace, when the trace ends in stuttering. For traces
     * ending in a lasso, see {@link EC#TLC_BACK_TO_STATE}. Note this code
     * only accompanies the *final* state in the trace, and the prefix
     * traces are accompanied by the {@link EC#TLC_STATE_PRINT2} code.
     */
    int TLC_STATE_PRINT3 = 2218;

    int TLC_SANY_END = 2219;
    int TLC_FAILED_TO_RECOVER_INIT = 2123;
    int TLC_REPORTER_DIED = 2124;

    int SYSTEM_ERROR_READING_POOL = 2125;
    int SYSTEM_CHECKPOINT_RECOVERY_CORRUPT = 2126;
    int SYSTEM_ERROR_WRITING_POOL = 2127;
    int SYSTEM_ERROR_CLEANING_POOL = 2270;
    int SYSTEM_INDEX_ERROR = 2134;
    int SYSTEM_STREAM_EMPTY = 2135;
    int SYSTEM_FILE_NULL = 2137;
    int SYSTEM_INTERRUPTED = 2138;
    int SYSTEM_UNABLE_NOT_RENAME_FILE = 2160;
    int SYSTEM_DISK_IO_ERROR_FOR_FILE = 2161;
    int SYSTEM_METADIR_EXISTS = 2162;
    int SYSTEM_METADIR_CREATION_ERROR = 2163;
    int SYSTEM_UNABLE_TO_OPEN_FILE = 2167;
    int TLC_BUG = 2128; // TODO Bad description
    int TLC_FINGERPRINT_EXCEPTION = 2147;

    int SYSTEM_DISKGRAPH_ACCESS = 2129; // TODO refactor

    int TLC_AAAAAAA = 2130;
    int TLC_REGISTRY_INIT_ERROR = 2131;
    int TLC_CHOOSE_ARGUMENTS_WRONG = 2164;
    int TLC_CHOOSE_UPPER_BOUND = 2165;

    int TLC_VALUE_ASSERT_FAILED = 2132;
	int TLC_MODULE_VALUE_JAVA_METHOD_OVERRIDE = 2154;
    int TLC_MODULE_VALUE_JAVA_METHOD_OVERRIDE_LOADED = 2168;
    int TLC_MODULE_VALUE_JAVA_METHOD_OVERRIDE_MISMATCH = 2400;
    int TLC_MODULE_VALUE_JAVA_METHOD_OVERRIDE_MODULE_MISMATCH = 2402;
    int TLC_MODULE_VALUE_JAVA_METHOD_OVERRIDE_IDENTIFIER_MISMATCH = 2403;
    // User-provided module overrides print to stdout.
    int TLC_MODULE_OVERRIDE_STDOUT = 20000;

    int TLC_FP_NOT_IN_SET = 2133;
    int TLC_FP_VALUE_ALREADY_ON_DISK = 2166;

    int TLC_LIVE_BEGRAPH_FAILED_TO_CONSTRUCT = 2159;
    int TLC_PARAMETER_MUST_BE_POSTFIX = 2136;
    int TLC_COULD_NOT_DETERMINE_SUBSCRIPT = 2139;
    int TLC_SUBSCRIPT_CONTAIN_NO_STATE_VAR = 2140;
    int TLC_WRONG_TUPLE_FIELD_NAME = 2141;
    int TLC_WRONG_RECORD_FIELD_NAME = 2142;
    int TLC_UNCHANGED_VARIABLE_CHANGED = 2143;
    int TLC_EXCEPT_APPLIED_TO_UNKNOWN_FIELD = 2144;

    int TLC_MODULE_TLCGET_UNDEFINED = 2145;
    /** Attempted to compare %1% with the value\n%2% */
    int TLC_MODULE_COMPARE_VALUE = 2155;
    int TLC_MODULE_CHECK_MEMBER_OF = 2158;
    int TLC_MODULE_TRANSITIVE_CLOSURE = 2157;
    /** The %1% argument of %2% should be a %3%, but instead it is:<br>%4% */
    int TLC_MODULE_ARGUMENT_ERROR = 2169;
    /** Simon used an argument like "\bn apple" to TLC_MODULE_ARGUMENT_ERROR to turn
     * an "a" into an "an".  This doesn't work on the Toolbox's console.  Hence, LL added
     * the following message type on 21 May 2012. */
    int TLC_MODULE_ARGUMENT_ERROR_AN = 2266;
    int TLC_MODULE_ONE_ARGUMENT_ERROR = 2283;
    int TLC_ARGUMENT_MISMATCH = 2170;
    int TLC_PARSING_FAILED2 = 2171;
    int TLC_PARSING_FAILED = 3002;
    int TLC_TOO_MNY_POSSIBLE_STATES = 2172;
    int TLC_ERROR_REPLACING_MODULES = 2173;
    int SYSTEM_ERROR_READING_STATES = 2174;
    int SYSTEM_ERROR_WRITING_STATES = 2175;
    int TLC_MODULE_APPLYING_TO_WRONG_VALUE = 2176;
    int TLC_MODULE_BAG_UNION1 = 2177;
    int TLC_MODULE_OVERFLOW = 2178;
    int TLC_MODULE_DIVISION_BY_ZERO = 2179;
    int TLC_MODULE_NULL_POWER_NULL = 2180;
    int TLC_MODULE_COMPUTING_CARDINALITY = 2181;
    int TLC_MODULE_EVALUATING = 2182;
    /** The %1% argument of %2% must be in the domain of its first argument:<br>%3%<br>, but instead it is<br>%4% */
    int TLC_MODULE_ARGUMENT_NOT_IN_DOMAIN = 2183;
    int TLC_MODULE_APPLY_EMPTY_SEQ = 2184;
    
    int TLC_SYMMETRY_SET_TOO_SMALL = 2300;
    int TLC_SPECIFICATION_FEATURES_TEMPORAL_QUANTIFIER = 2301;
    
    int TLC_STARTING = 2185;
    int TLC_FINISHED = 2186;
    
    // distributed TLC
    
    int TLC_DISTRIBUTED_SERVER_RUNNING = 7000;
    int TLC_DISTRIBUTED_WORKER_REGISTERED = TLC_DISTRIBUTED_SERVER_RUNNING + 1;
    int TLC_DISTRIBUTED_WORKER_DEREGISTERED = TLC_DISTRIBUTED_WORKER_REGISTERED + 1;
    int TLC_DISTRIBUTED_WORKER_STATS = TLC_DISTRIBUTED_WORKER_DEREGISTERED + 1;
    int TLC_DISTRIBUTED_SERVER_NOT_RUNNING = TLC_DISTRIBUTED_WORKER_STATS + 1;
    int TLC_DISTRIBUTED_VM_VERSION = TLC_DISTRIBUTED_SERVER_NOT_RUNNING + 1;
    int TLC_DISTRIBUTED_WORKER_LOST = TLC_DISTRIBUTED_VM_VERSION + 1;
    int TLC_DISTRIBUTED_EXCEED_BLOCKSIZE = TLC_DISTRIBUTED_WORKER_LOST + 1;
    int TLC_DISTRIBUTED_SERVER_FPSET_WAITING = TLC_DISTRIBUTED_EXCEED_BLOCKSIZE + 1;
    int TLC_DISTRIBUTED_SERVER_FPSET_REGISTERED = TLC_DISTRIBUTED_SERVER_FPSET_WAITING + 1;
    int TLC_DISTRIBUTED_SERVER_FINISHED = TLC_DISTRIBUTED_SERVER_FPSET_REGISTERED + 1;
    
    // errors during parsing of the model configuration
    
    int CFG_ERROR_READING_FILE = 5001;
    int CFG_GENERAL = 5002;
    int CFG_MISSING_ID = 5003;
    int CFG_TWICE_KEYWORD = 5004;
    int CFG_EXPECT_ID = 5005;
    int CFG_EXPECTED_SYMBOL = 5006;
    int TLC_MODE_MC = 2187;
    int TLC_MODE_MC_DFS = 2271;
    int TLC_MODE_SIMU = 2188;
    int TLC_COMPUTING_INIT = 2189;
	int TLC_COMPUTING_INIT_PROGRESS = 2269;
    int TLC_INIT_GENERATED1 = 2190;
    int TLC_INIT_GENERATED2 = 2191;
    int TLC_INIT_GENERATED3 = 2207;
    int TLC_INIT_GENERATED4 = 2208;
    int TLC_CHECKING_TEMPORAL_PROPS = 2192;
    int TLC_CHECKING_TEMPORAL_PROPS_END = 2267;
    int TLC_SUCCESS = 2193;
    int TLC_SEARCH_DEPTH = 2194;
    int TLC_STATE_GRAPH_OUTDEGREE = 2268;
    int TLC_CHECKPOINT_START = 2195;
    int TLC_CHECKPOINT_END = 2196;
    int TLC_CHECKPOINT_RECOVER_START = 2197;
    int TLC_CHECKPOINT_RECOVER_END = 2198;
    int TLC_STATS = 2199;
    int TLC_STATS_DFID = 2204;
    int TLC_STATS_SIMU = 2210;
    int TLC_PROGRESS_STATS = 2200;
    int TLC_COVERAGE_START = 2201;
    int TLC_COVERAGE_END = 2202;
    int TLC_CHECKPOINT_RECOVER_END_DFID = 2203;
    int TLC_PROGRESS_START_STATS_DFID = 2205;
    int TLC_PROGRESS_STATS_DFID = 2206;
    int TLC_PROGRESS_SIMU = 2209;
    int TLC_FP_COMPLETED = 2211;
    
    int TLC_LIVE_IMPLIED = 2212;
    int TLC_LIVE_CANNOT_HANDLE_FORMULA = 2213;
    int TLC_LIVE_WRONG_FORMULA_FORMAT = 2214;
    int TLC_LIVE_ENCOUNTERED_ACTIONS = 2249;
    int TLC_LIVE_STATE_PREDICATE_NON_BOOL = 2250;
    int TLC_LIVE_CANNOT_EVAL_FORMULA = 2251;
    int TLC_LIVE_ENCOUNTERED_NONBOOL_PREDICATE = 2252;
    int TLC_LIVE_FORMULA_TAUTOLOGY = 2253;

    
    int TLC_EXPECTED_VALUE = 2215;
    int TLC_EXPECTED_EXPRESSION = 2246;
    int TLC_EXPECTED_EXPRESSION_IN_COMPUTING = 2247;
    int TLC_EXPECTED_EXPRESSION_IN_COMPUTING2 = 2248;
    
    
    /**
     * This error code is used in the following situations:
     *  - During DFID model checking, when next state is not fully defined
     *  - In the {@link tlc2.tool.CheckImpl} tool, when there is an invalid step
     *  - During Simulation model checking, when maximum trace depth is reached
     */
    int TLC_STATE_PRINT1 = 2216;
    
    /**
     * This error code is used in the following situations:
     *  - Printing a safety invariant violation error trace
     *  - Printing every state except the final state of a liveness error trace; the final state is printed with:
     *    * {@link EC#TLC_BACK_TO_STATE} for liveness traces ending in a lasso
     *    * {@link EC#TLC_STATE_PRINT3} for liveness traces ending in stuttering
     */
    int TLC_STATE_PRINT2 = 2217;
    
    int TLC_SANY_START = 2220;
    int TLC_COVERAGE_MISMATCH = 2776;
    int TLC_COVERAGE_VALUE = 2221;
    int TLC_COVERAGE_VALUE_COST = 2775;
    int TLC_COVERAGE_NEXT = 2772;
    int TLC_COVERAGE_INIT = 2773;
    int TLC_COVERAGE_PROPERTY = 2774;
    int TLC_COVERAGE_CONSTRAINT = 2778;
    int TLC_COVERAGE_END_OVERHEAD = 2777;
    
    // config file errors
    int TLC_CONFIG_VALUE_NOT_ASSIGNED_TO_CONSTANT_PARAM = 2222;
    int TLC_CONFIG_RHS_ID_APPEARED_AFTER_LHS_ID = 2223;
    int TLC_CONFIG_WRONG_SUBSTITUTION = 2224;
    int TLC_CONFIG_WRONG_SUBSTITUTION_NUMBER_OF_ARGS = 2225;
    int TLC_CONFIG_UNDEFINED_OR_NO_OPERATOR = 2280;
    int TLC_CONFIG_SUBSTITUTION_NON_CONSTANT = 2281;
    int TLC_CONFIG_ID_DOES_NOT_APPEAR_IN_SPEC = 2226;
    int TLC_CONFIG_NOT_BOTH_SPEC_AND_INIT = 2227;
    int TLC_CONFIG_ID_REQUIRES_NO_ARG = 2228;
    int TLC_CONFIG_SPECIFIED_NOT_DEFINED = 2229;
    int TLC_CONFIG_ID_HAS_VALUE = 2230;
    int TLC_CONFIG_MISSING_INIT = 2231;
    int TLC_CONFIG_MISSING_NEXT = 2232;
    int TLC_CONFIG_ID_MUST_NOT_BE_CONSTANT = 2233;
    int TLC_CONFIG_OP_NO_ARGS = 2234;
    int TLC_CONFIG_OP_NOT_IN_SPEC = 2235;
    int TLC_CONFIG_OP_IS_EQUAL = 2236;
    int TLC_CONFIG_SPEC_IS_TRIVIAL = 2237;
    int TLC_CANT_HANDLE_SUBSCRIPT = 2238;
    int TLC_CANT_HANDLE_CONJUNCT = 2239;
    int TLC_CANT_HANDLE_TOO_MANY_NEXT_STATE_RELS = 2240;
    int TLC_CONFIG_PROPERTY_NOT_CORRECTLY_DEFINED = 2241;
    int TLC_CONFIG_OP_ARITY_INCONSISTENT = 2242;
    int TLC_CONFIG_NO_STATE_TYPE = 2243;
    int TLC_CANT_HANDLE_REAL_NUMBERS = 2244;
    int TLC_NO_MODULES = 2245;
    
    int TLC_ENABLED_WRONG_FORMULA = 2260;
    int TLC_ENCOUNTERED_FORMULA_IN_PREDICATE = 2261;
    int TLC_VERSION = 2262;
    int TLC_COUNTER_EXAMPLE = 2264;
    
    int TLC_INTEGER_TOO_BIG = 2265;
    int TLC_TRACE_TOO_LONG = 2282;
    
    int TLC_ENVIRONMENT_JVM_GC = 2401;

    // Codes for trace expression spec generation events
    int TLC_TE_SPEC_GENERATION_COMPLETE = 2501;
    int TLC_TE_SPEC_GENERATION_ERROR = 2502;
    
    //**************************************************************//
    // Mapping error constants above to process exit/return values. //
    // Because Linux and macOS only support 8-bit exit values, this //
    // mapping is necessary.                                        //
    //**************************************************************//

    class ExitStatus {
	    	
    	public static final int ERROR = 255;
	    public static final int SUCCESS = 0;
	
	    // (Safety/Liveness) Violations
	    public static final int VIOLATION_ASSUMPTION = 10; 
	    public static final int VIOLATION_DEADLOCK = VIOLATION_ASSUMPTION + 1; 
	    public static final int VIOLATION_SAFETY = VIOLATION_DEADLOCK + 1;
	    public static final int VIOLATION_LIVENESS = VIOLATION_SAFETY + 1;
		public static final int VIOLATION_ASSERT = VIOLATION_LIVENESS + 1;
	
	    // Evaluation failures
	    public static final int FAILURE_SPEC_EVAL = 75;
	    public static final int FAILURE_SAFETY_EVAL = FAILURE_SPEC_EVAL + 1;
	    public static final int FAILURE_LIVENESS_EVAL = FAILURE_SAFETY_EVAL + 1;
	    
	    // Errors
	    public static final int ERROR_SPEC_PARSE = 150;
	    public static final int ERROR_CONFIG_PARSE = ERROR_SPEC_PARSE + 1;
	    public static final int ERROR_STATESPACE_TOO_LARGE = ERROR_CONFIG_PARSE + 1;
	    public static final int ERROR_SYSTEM = ERROR_STATESPACE_TOO_LARGE + 1;
	
	    /**
	     * Returns an exit status for an error code.
	     */
	    public static int errorConstantToExitStatus(final int ec) {
	        // TODO Allocate a range of exit status to indicate classes of errors.
	        // For a great example of potential classes see: https://github.com/tlaplus/tlaplus/pull/308#discussion_r285840112
            return switch (ec) {
                case NO_ERROR -> SUCCESS;

                // failures

                case TLC_LIVE_FORMULA_TAUTOLOGY -> FAILURE_LIVENESS_EVAL;
                case TLC_STATE_NOT_COMPLETELY_SPECIFIED_NEXT, TLC_STATES_AND_NO_NEXT_ACTION, TLC_NESTED_EXPRESSION, TLC_FINGERPRINT_EXCEPTION ->
                        FAILURE_SPEC_EVAL;
                case TLC_INVARIANT_EVALUATION_FAILED, TLC_INVARIANT_VIOLATED_LEVEL -> FAILURE_SAFETY_EVAL;

                // violations

                case TLC_INVARIANT_VIOLATED_INITIAL, TLC_INVARIANT_VIOLATED_BEHAVIOR -> VIOLATION_SAFETY;
                case TLC_ACTION_PROPERTY_VIOLATED_BEHAVIOR, TLC_ACTION_PROPERTY_EVALUATION_FAILED, TLC_TEMPORAL_PROPERTY_VIOLATED, TLC_PROPERTY_VIOLATED_INITIAL ->
                        VIOLATION_LIVENESS;
                case TLC_DEADLOCK_REACHED -> VIOLATION_DEADLOCK;
                case TLC_ASSUMPTION_FALSE, TLC_ASSUMPTION_EVALUATION_ERROR -> VIOLATION_ASSUMPTION;
                case TLC_VALUE_ASSERT_FAILED -> VIOLATION_ASSERT;

                // errors
                // might also be in the spec
                case TLC_CONFIG_VALUE_NOT_ASSIGNED_TO_CONSTANT_PARAM, TLC_CONFIG_RHS_ID_APPEARED_AFTER_LHS_ID, TLC_CONFIG_WRONG_SUBSTITUTION, TLC_CONFIG_WRONG_SUBSTITUTION_NUMBER_OF_ARGS, TLC_CONFIG_UNDEFINED_OR_NO_OPERATOR, TLC_CONFIG_SUBSTITUTION_NON_CONSTANT, TLC_CONFIG_ID_DOES_NOT_APPEAR_IN_SPEC, TLC_CONFIG_NOT_BOTH_SPEC_AND_INIT, TLC_CONFIG_ID_REQUIRES_NO_ARG, TLC_CONFIG_SPECIFIED_NOT_DEFINED, TLC_CONFIG_ID_HAS_VALUE, TLC_CONFIG_MISSING_INIT, TLC_CONFIG_MISSING_NEXT, TLC_CONFIG_ID_MUST_NOT_BE_CONSTANT, TLC_CONFIG_OP_NO_ARGS, TLC_CONFIG_OP_NOT_IN_SPEC, TLC_CONFIG_OP_IS_EQUAL, TLC_CONFIG_SPEC_IS_TRIVIAL, TLC_CANT_HANDLE_SUBSCRIPT, TLC_CANT_HANDLE_CONJUNCT, TLC_CANT_HANDLE_TOO_MANY_NEXT_STATE_RELS, TLC_CONFIG_PROPERTY_NOT_CORRECTLY_DEFINED, TLC_CONFIG_OP_ARITY_INCONSISTENT, TLC_CONFIG_NO_STATE_TYPE, TLC_CANT_HANDLE_REAL_NUMBERS, TLC_NO_MODULES ->
                        ERROR_CONFIG_PARSE;
                case TLC_PARSING_FAILED2, TLC_PARSING_FAILED -> ERROR_SPEC_PARSE;
                default -> 255;
            };
	    }

		private static final Set<Integer> knownExitValues = Stream.of(SUCCESS, FAILURE_LIVENESS_EVAL, FAILURE_SPEC_EVAL,
				FAILURE_SAFETY_EVAL, VIOLATION_SAFETY, VIOLATION_LIVENESS, VIOLATION_DEADLOCK, VIOLATION_ASSUMPTION,
				VIOLATION_ASSERT, ERROR_CONFIG_PARSE, ERROR_SPEC_PARSE).collect(Collectors.toSet());
		
		public static boolean exitStatusToCrash(final int exitStatus) {
			return !knownExitValues.contains(exitStatus);
		}
    }
}
