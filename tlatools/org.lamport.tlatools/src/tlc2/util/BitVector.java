...
public class BitVector implements Serializable {
   ...
   */
  public BitVector(final int initCapacity) {
    final int len = (initCapacity == 0) ? 0 : ((initCapacity - 1)/64 + 1);
	 ...
	 * to <code>true</code>
	 */
	public BitVector(final int initCapacity, final boolean initValue) {
		final int len = (initCapacity == 0) ? 0 : ((initCapacity - 1) / 64 + 1);
		this.word = new long[len];
		if (initValue) {
			set(0, len);
  ...
  public boolean equals(final Object o) {
    ...
    if (this.word.length != other.word.length) {
      final int maxLen = Math.max(this.word.length, other.word.length);
      final long[] tail = (maxLen == this.word.length) ? this.word : other.word;
  ...
  /** Set all the bits with indices in the closed interval
      <code>[lo, hi]</code>. */
  public void set(final int lo, final int hi) {
    final int lwd = lo / 64;
    final int hwd = hi / 64;
    if (hwd >= this.word.length) this.grow(hwd);
    final int lbit = lo % 64;
    final int hbit = hi % 64;
    if (lwd < hwd) {
  ...
  public String toString(final int start, final int length, final char one, final char zero) {
  ...
  public void read(final BufferedRandomAccessFile raf) throws IOException {
    final int len = raf.readNat();
    this.word = new long[len];
    for (int i = 0; i < len; i++) {
  ...
  /** Grow this bit vector to contain at least <code>wd+1</code>
      words. */
  private void grow(final int wd) {
    // Assert.check(wd >= this.word.length);
    final int newLen = Math.max(this.word.length, wd + 1);
    final long[] tmp = new long[newLen];
  ...
  public static class Iter {
	...
	over <code>bv</code>. */
    public Iter(final BitVector bv) { this.init(bv); }
        
    /** Reinitialize this <code>BVIter</code> object for iterating
	over <code>bv</code>. */
    public void init(final BitVector bv) {
      this.word = bv.word;
      this.wd = 0;
      this.bit = 0;
      this.mask = 1L;
	...
	final long w = this.word[this.wd];
	for (; this.bit < 64; this.bit++, this.mask <<= 1) {
	  if ((w & this.mask) != 0L) {
	    final int res = (this.wd * 64) + this.bit;
